...
% configuración para usar la tipografía Brill
\usepackage{fontspec}
\protrudechars=2 %    \pdfprotrudechars=2 and
...
%\usepackage[final,babel=true]{microtype}% Mejoras tipográficas micro
...
%\newfontfamily\minionCond{Minion Pro}
%[Numbers={OldStyle,Proportional},Ligatures=TeX,
%Scale=1.2,
%SizeFeatures={
%        {Size={-8.40},Font=* CnCapt},
%        {Size={8.41-14.0},Font=* Cn},
%        {Size={14.01-24.0},Font=* CnSubh},
%        {Size={24.01-},Font=* CnDisp}},
%UprightFont=*-Cn,
%ItalicFont=*-CnIt,
%BoldFont=*-SemiboldCn,
%BoldItalicFont=*-SemiboldCnIt]
...
% Formato simple y directo con titleformat
\titleformat{\chapter}[display]
  {\normalfont} % Formato del título
  {% Formato del número (etiqueta)
    \ifnum\value{chapter}>0
      \begin{center}
        \begin{tikzpicture}
          \node[circle, draw=none, fill=chaptercircle, minimum size=2cm, inner sep=0pt]
            {\makebox[2cm][c]{\color{black!70}\bfseries\fontsize{48pt}{60pt}\selectfont\thechapter}};
        \end{tikzpicture}
      \end{center}
    \else
      % Para capítulos no numerados, crear un espacio del mismo tamaño
      \begin{center}
        \rule{0pt}{2cm} % Altura aproximada del círculo con número
      \end{center}
    \fi
  }
  {0.5cm} % Espacio entre el número/etiqueta y el título
  {\filright\LARGE} % Formato aplicado al título

% Control adicional del espaciado
\titlespacing*{\chapter}
  {} % Espacio izquierdo
  {} % Espacio vertical antes del capítulo
  {2.5cm} % Espacio vertical después del capítulo
